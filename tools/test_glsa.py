...
from metrics import eval_map, ltAnalysis
...
def main():
    ...
    if rank == 0:
        ...
        mAP_val, per_class_AP = eval_map(scores, gt_labels)

        print(f"\nmAP (custom eval_map): {mAP_val:.4f}")
        print("\nPer-class AP:")
        for idx, ap in enumerate(per_class_AP):
            class_name = CLASSES[idx] if idx < len(CLASSES) else f"class_{idx}"
            print(f"{class_name}: {ap:.4f}")

        dataset_type = 'coco-lt' if 'coco' in args.config.lower() else 'voc-lt'
        head_AP, mid_AP, tail_AP, head, mid, tail = ltAnalysis(per_class_AP, dataset_type)
        print(f"\nHead mAP: {head:.2f}, Middle mAP: {mid:.2f}, Tail mAP: {tail:.2f}")

        import pandas as pd
        pd.DataFrame({'Class': CLASSES, 'AP': per_class_AP}).to_csv('per_class_ap.csv', index=False)
        pd.DataFrame({'Head_AP': head_AP, 'Mid_AP': mid_AP,'Tail_AP': tail_AP,'Head': head, 'Mid': mid,'Tail': tail}).to_csv('TL_ap.csv', index=False)
